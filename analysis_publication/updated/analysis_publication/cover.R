...
mix.canopy$management<-factor(mix.canopy$management, levels=c("unmanaged", "flailed", "scraped"))
...
TukeyHSD(aov(data=subset(mix.canopy, seedmix=="annuals"|seedmix=="perennials"|seedmix=="industry"), cov~as.factor(management)*as.factor(year)))

test<-subset(mix.canopy, seedmix=="annuals"|seedmix=="perennials"|seedmix=="industry")%>%
    mutate(trt=as.factor(paste(seedmix, year, management,  sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

test<-subset(mix.canopy, seedmix=="annuals"|seedmix=="perennials"|seedmix=="industry")%>%
  mutate(trt=as.factor(paste(seedmix, orchard_age,  sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

TukeyHSD(mod)

mod<-lme(data=subset(test), cov~as.factor(orchard_age), random=~1|year/block)

cld(glht(mod, mcp(trt="Tukey")))
 emmeans(mod, pairwise~orchard_age)
...
#winterbare
 winter.canopy.rel<-winter.canopy%>%
   ...
   mutate(industrydiff=(industry)-(control))%>%
   select(-(6:9))%>%
   gather(seedmix, wintervegveg, annualsdiff, perennialsdiff, industrydiff)
...
#NEW SUPPLEMENT - raw weeds and bare in relation to canopy
ggplot(subset(bare.canopy, !is.na(seedmix)&species=="bare"|species=="weeds"&management!="orchard"),aes(x=canopy, y=cov))+
  geom_point( size=1)+
 # geom_smooth(se=T, method = "lm", formula = y ~ x, size = 1, color="black")+
  scale_color_manual(values=c("black", "grey40", "grey70"))+
  ylab(" % cover ground spring")+xlab("")+ xlab("% Canopy cover")+ facet_grid(species~year, scales="free")+ theme(axis.line=element_line())+geom_hline(aes(yintercept=0))+
  geom_smooth(method = "lm", se=FALSE, color="black", formula = y ~ x)  +
  stat_regline_equation(label.y=110)+stat_cor(label.y=120)
'library(ggpubr)


test<-subset(bare.canopy, seedmix=="annuals"|seedmix=="perennials"|seedmix=="industry"|seedmix=="control")%>%
  mutate(trt=as.factor(paste(seedmix, year,  sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

mod<-lme(data=subset(test, species=="weeds"), cov~trt, random=~1|orchard_age/block)
emmeans(mod, pairwise~trt)
cld(glht(mod, mcp(trt="Tukey")))

test<-subset(bare.canopy.rel, seedmix=="annualsdiff"|seedmix=="perennialsdiff"|seedmix=="industrydiff")%>%
  mutate(trt=as.factor(paste(seedmix, year,  sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

summary(lme(data=subset(test, year==2021&seedmix=="perennialsdiff"&species=="weeds"&!is.na(cov)&!is.na(canopy)), cov~canopy, random=~1|block))



#winterveg
ggplot(subset(winter.canopy.rel, !is.na(seedmix)),aes(x=canopy, y=wintervegveg))+
  #geom_point(aes(shape=as.factor(year), color=as.factor(management)), size=1) + 
  #stat_smooth(aes(linetype=as.factor(year), color=as.factor(management)),method = "lm", se=F, formula = y ~ poly(x, 2), size = 1)+
  geom_point(aes(shape=as.factor(year)))+
  geom_smooth(data=subset(winter.canopy.rel, seedmix=="perennialsdiff"),aes(linetype=as.factor(year)),size = 1, method="lm", formula=y~(x), se=F, color="black")+
  # geom_smooth(data=subset(bare.canopy,seedmix!="megamix"&!is.na(seedmix)), aes(color=management, linetype=as.factor(year)), se=F, size = 1,  method = "gam", formula = y ~ s(x),)+
  # stat_smooth(aes(linetype=as.factor(year), color=as.factor(management)), se=F, size = 1,  method = "gam", formula = y ~ s(x))+
  scale_color_manual(values=c("black", "grey40", "grey70"))+
  ylab("Difference in winter % vegetative cover")+xlab("")+facet_wrap(~seedmix, scales="free")+xlab("% Canopy cover") +
  scale_x_continuous(limits=c(0,100)) + scale_y_continuous(limits=c(-100,60))+ theme(axis.line=element_line())+geom_hline(aes(yintercept=0))


test<-subset(winter.canopy, seedmix=="annuals"|seedmix=="perennials"|seedmix=="industry"|seedmix=="control")%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

mod<-lme(data=subset(test), veg~seedmix, random=~1|block)
emmeans(mod, pairwise~seedmix)
cld(glht(mod, mcp(trt="Tukey")))

test<-subset(winter.canopy.rel, seedmix=="annualsdiff"|seedmix=="perennialsdiff"|seedmix=="industrydiff")%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

summary(lme(data=subset(test, seedmix=="industrydiff"&!is.na(wintervegveg)&!is.na(canopy)), wintervegveg~canopy, random=~1|id))
...
veg.canopy$species<-factor(veg.canopy$species, levels=c("amsinckia", "clarkia", "collomia", "epilobium", "gilia", "plectritis", "achillea", "eriophyllum", "geum", "lomatium", "potentilla", "prunella", "barley", "oats", "clover", "vetch","agoseris", "lotus", "viola", "sanguisorba"))
...
grid.arrange(
  ggplot(subset(veg.canopy, species%in%c("amsinckia", "clarkia", "collomia", "lotus")), aes(x=canopy, y=plotcover))+ ## CHECK OUT WHAT IS GOING ON WITH INDUSTRY
    ...
    scale_color_manual(values=c("black", "grey40", "grey70"))+
    ylab("Target % Cover")+xlab(""),
  ggplot(subset(veg.canopy, species%in%c("epilobium", "gilia", "plectritis", "sanguisorba")), aes(x=canopy, y=plotcover))+ ## CHECK OUT WHAT IS GOING ON WITH INDUSTRY
    geom_point(aes(shape=as.factor(year))) + 
    scale_shape_manual(values=c(19, 1))+
    #poly
    stat_smooth(data=as.data.frame(subset(veg.canopy, 
...
#annuals
summary(lme(plotcover~poly(canopy,2), random=~1|block, data=as.data.frame(subset(veg.canopy, species==("amsinckia")&year==2020&!is.na(canopy))))) #poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("amsinckia")&year==2021&!is.na(canopy))))) #poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("clarkia")&year==2020&!is.na(canopy))))) #poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("clarkia")&year==2021&!is.na(canopy))))) #lin
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("collomia")&year==2020&!is.na(canopy))))) #poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("collomia")&year==2021&!is.na(canopy))))) # poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("lotus")&year==2020&!is.na(canopy)))))# nothing
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("lotus")&year==2021&!is.na(canopy))))) #lin

summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("epilobium")&year==2020&!is.na(canopy))))) #poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("epilobium")&year==2021&!is.na(canopy))))) #lin
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("gilia")&year==2020&!is.na(canopy))))) #lin
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("gilia")&year==2021&!is.na(canopy))))) #lin
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("plectritis")&year==2020&!is.na(canopy))))) #lin
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("plectritis")&year==2021&!is.na(canopy))))) #poly

summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("sanguisorba")&year==2020&!is.na(canopy))))) #lin
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("sanguisorba")&year==2021&!is.na(canopy))))) #poly
...
#perennials
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("achillea")&year==2020&!is.na(canopy))))) #nothing 
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("achillea")&year==2021&!is.na(canopy))))) #linear 
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("eriophyllum")&year==2020&!is.na(canopy))))) #poly 
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("eriophyllum")&year==2021&!is.na(canopy))))) #linear 
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("geum")&year==2020&!is.na(canopy))))) #poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("geum")&year==2021&!is.na(canopy))))) # poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("agoseris")&year==2020&!is.na(canopy)))))# poly
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("agoseris")&year==2021&!is.na(canopy))))) #nothing
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("lomatium")&year==2020&!is.na(canopy)))))# linear
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("lomatium")&year==2021&!is.na(canopy))))) #linear
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("potentilla")&year==2020&!is.na(canopy)))))# nothing
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("potentilla")&year==2021&!is.na(canopy))))) #nothing
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("prunella")&year==2020&!is.na(canopy)))))# nothing
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("prunella")&year==2021&!is.na(canopy))))) #linear
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("viola")&year==2020&!is.na(canopy)))))# linear
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("viola")&year==2021&!is.na(canopy))))) #nothing
...
grid.arrange(
  ...
  ggplot(subset(veg.canopy, species%in%c("clover", "vetch")), aes(x=canopy, y=plotcover))+ ## CHECK OUT WHAT IS GOING ON WITH INDUSTRY
    ...
    stat_smooth(data=subset(veg.canopy, species=="clover"&year==2021), aes(linetype=as.factor(year)), method = "lm", se=F, formula = y ~ x, size = 1, color="black")+
   # stat_smooth(data=subset(veg.canopy, species=="vetch"&year==2021), aes(linetype=as.factor(year)), method = "lm", se=F, formula = y ~ x, size = 1, color="white")+
   # stat_smooth(data=subset(veg.canopy, species=="clover"&year==2020), aes(linetype=as.factor(year)), method = "lm", se=F, formula = y ~ x, size = 1, color="white")+
...
#industry
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("barley")&year==2020&!is.na(canopy))))) #nothing significant
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("barley")&year==2021&!is.na(canopy))))) #linear p=.004
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("clover")&year==2020&!is.na(canopy))))) #nothing significant
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("clover")&year==2021&!is.na(canopy))))) #lin p=3.13 -05
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("oats")&year==2020&!is.na(canopy))))) #nothing
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("oats")&year==2021&!is.na(canopy))))) # nothing
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("vetch")&year==2020&!is.na(canopy)))))# no lin, poly=.03
summary(lme(plotcover~poly(canopy,2),random=~1|block,data=as.data.frame(subset(veg.canopy, species==("vetch")&year==2021&!is.na(canopy))))) #nothing
...
#summarize veg canopy into average covers by year
veg.cansum<-veg.canopy%>%
  group_by(year, species)%>%
  summarize(meancov=mean(plotcover), secov=calcSE(plotcover))
vct<-veg.canopy%>%mutate(trt=as.factor(paste(year, species)))
mod<-lme(data=subset(vct, !is.na(species)), plotcover~trt)
TukeyHSD(aov(data=subset(vct, !is.na(species)&species=="clover"), plotcover~trt))

cld(glht(mod, mcp(trt="Tukey")))

covsum<-cover%>%
#  filter(species %in% c("epilobium", "clarkia", "collomia", "amsinckia", "gilia", "sanguisorba", "plectritis", "lotus"))%>%
  filter(seedmix!="megamix")%>%
  group_by(orchard_age, species, year, management, block)%>%
  summarize(cov=sum(cov))%>%
  spread(species, cov, fill=0)%>%
  gather(species, cov, 5:25)%>%
