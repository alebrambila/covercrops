...
ggplot(moisture21_calibration, aes(x=nail, y=probe))+ geom_point(aes(color=date))+geom_smooth(method="lm")+
  ...
  xlab("%VWC (Probe)")+ylab("%VWC (Nail)")+ theme_classic()+ stat_regline_equation()+stat_cor(label.y=40)

#2020 stats
mod<-lme(data=moisture21_calibration, nail~probe, random=~1|orchardage/block)
mod<-lm(data=moisture21_calibration, nail~probe)
...
#2020 and 2021 JUST 40/60 YEAR ORCHARD - PUBLICATION FIG
ggplot(subset(binall_2020, seedmix!="megamix"), aes(x=week, y=meancal)) +
 ...
 scale_y_continuous(limits=c(5,47))+ theme(axis.line=element_line())+
  labs(x="week", y="Soil % Water by Volume")+theme(axis.text.x=element_text(color = "black", angle=30, vjust=.8, hjust=0.8))+
  scale_color_manual(labels=c("annuals", "control", "conventional", "perennial"),values=c("#E7298A","#D95F02","#7570B3",   "#1B9E77"))+
scale_x_continuous(breaks=c(1, 2, 3, 4, 5, 6), labels=c("april 7","", "april 24", "may 1", "may 10", "may 17"))
ggplot(aes(x=month, y=meancal), data=subset(moisture21cal, !seeding%in% c("true control", "north", "south", "Megamix"))) +
  geom_jitter(data=subset(moisture21bin, orchardage!=15&!seeding%in% c("true control", "north", "south", "Megamix")), aes(x=month, y=calibration, color=seeding), size=.75, width=.2, alpha=.5)+
  geom_point(aes(color=seeding)) +
  geom_line(aes(color=seeding))+
  geom_errorbar(aes(color=seeding, ymin=meancal-secal, ymax=meancal+secal), width=.2)+
scale_y_continuous(limits=c(5,47))+ theme(axis.line=element_line())+
  labs(x="month", y="Soil % Water by Volume")+theme(axis.text.x=element_text(color = "black", angle=30, vjust=.8, hjust=0.8))+
  scale_color_manual(values=c("#F8766D",  "orange",  "#00BA38", "#619CFF"))+
...
#2020 stats
test<-subset(moisture20bin, orchard_age!=15&seedmix!="megamix")%>%
  mutate(trt=as.factor(paste(seedmix, week,  sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

mod<-lme(data=test, moisture~seedmix*week, random=~1|orchard_age/block)
summary(mod)
emmeans(mod, pairwise~seedmix)

#seedmix    emmean   SE df lower.CL upper.CL
#annuals      34.8 1.23  1     19.2     50.5
#control      31.9 1.23  1     16.3     47.6
#industry     31.7 1.25  1     15.7     47.6
#perennials   33.7 1.23  1     18.0     49.3
#contrast              estimate    SE  df t.ratio p.value
#annuals - control        2.868 0.458 587  6.260  <.0001 
#annuals - industry       3.144 0.512 587  6.137  <.0001 
#annuals - perennials     1.112 0.458 587  2.427  0.0731 
#control - industry       0.276 0.512 587  0.538  0.9497 
#control - perennials    -1.756 0.458 587 -3.833  0.0008 
#industry - perennials   -2.032 0.512 587 -3.966  0.0005 

mod<-lme(data=subset(test, week==6), moisture~seedmix, random=~1|orchard_age/block)
emmeans(mod, pairwise~seedmix)
...
mod<-lme(data=subset(test), cov~trt, random=~1|orchard_age/block)
...
test<-subset(moisture21bin, orchard_age!=15&seedmix!="megamix"&seedmix!="true control")%>%
  mutate(trt=as.factor(paste(seedmix, month,  sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block,sep="_")))

mod<-lme(data=subset(test, !is.na(calibration)), calibration~seedmix*month, random=~1|orchard_age/block)
summary(mod)
emmeans(mod, pairwise~seedmix)
#no main or interactive effects, but...:
#contrast              estimate    SE  df t.ratio p.value
#annuals - control       -0.846 0.475 640 -1.782  0.2831 
#annuals - industry      -1.852 0.532 640 -3.481  0.0030 
#annuals - perennials    -0.350 0.475 640 -0.736  0.8824 
##control - industry      -1.006 0.532 640 -1.891  0.2329 
#control - perennials     0.496 0.475 640  1.045  0.7228 
#industry - perennials    1.503 0.532 640  2.824  0.0252 


mod<-lme(data=subset(test, month==8), calibration~seedmix, random=~1|orchard_age/block)
emmeans(mod, pairwise~seedmix)
