...
phenflor_abun<-phenflor_spread%>%
  ...
  mutate(id=paste(orchardage, block, management, seedmix, sep="_"))%>%
  spread(date, infloresences, fill=0)%>%
  gather(date, infloresences, 6:10)%>%
  filter((seedmix=="industry"&management!="unmanaged")|seedmix=="annuals"|seedmix=="perennials")


phenflor_abun$orchardage<-factor(phenflor_abun$orchardage, levels=c("15", "60", "40"))

plotkey1<-plotkey%>%
  mutate(orchardage=as.factor(orchard_age))%>%
  ungroup()%>%
  filter(seedmix!="megamix")%>%
  dplyr::select(-orchard_age)

test<-full_join(plotkey1, phenflor_abun)%>%
  group_by(orchardage, block, date, seedmix)%>%
  summarize(count=n())

### 1/3

ggplot(data=subset(phenflor_abun,   management=="unmanaged"|(management!="unmanaged"&date<7)), aes(x=as.numeric(date), y=infloresences+1, color=as.factor(seedmix)))+
  ...
  geom_jitter(width=.2, alpha=.5, size=.7)+
  ylab("floral abundance per plot: managed")+
  stat_summary(data=subset(phenflor_abun,   (management=="unmanaged"|(management!="unmanaged"&date<7))&date<7), geom="line", aes(color=seedmix), fun="mean", size=1)+
  stat_summary(data=subset(phenflor_abun,   (management=="unmanaged"|(management!="unmanaged"&date<7))&date>5), geom="line", aes(color=seedmix), linetype=2, fun="mean", size=1)+
  stat_summary(geom="point", aes(color=seedmix), fun="mean", size=1.5)+
...
#model 1: within/across orchards seed mixes main effects
test<-subset(phenflor_abun,   management=="unmanaged"|(management!="unmanaged"&date<7))%>%
  mutate(trt=as.factor(paste(seedmix, orchardage, sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchardage, block, date, sep="_")))%>%
  filter(management=="unmanaged"|(management!="unmanaged"&date<7))

mod<-lme(data=subset(test), infloresences~trt, random=~1|date/block)
...
#test 2: date by date
test<-subset(phenflor_abun,   management=="unmanaged"|(management!="unmanaged"&date<7))%>%
  mutate(trt=as.factor(paste( date, sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchardage, block, sep="_")))%>%
  mutate(date=as.factor(date))

mod<-lme(data=subset(test, orchardage==60&seedmix=="industry"), infloresences~date, random=~1|block)

cld(glht(mod, mcp(date="Tukey")))
emmeans(mod, pairwise~date)
...
ggplot(subset(phenology_xl, species=="<NA>"), aes(x = date, y=proportion, fill = stage)) +
  geom_density(position = "fill",  stat="identity") +
  #scale_fill_manual(values=c("lightgreen", "darkgreen", "goldenrod","darkorange", "grey80", "grey30")) +
...
poll_divsum<-poll_div1%>%
  ...
  mutate(rem=paste(management, seedmix, sep="_"))%>%
  filter(rem!="unmanaged_industry")
...
ggplot(subset(poll_divsum, management=="unmanaged"|(management!="unmanaged"&numericmonth<7)), aes(x=as.numeric(numericmonth), y=richness, color=as.factor(seedmix)))+
  ...
  stat_summary(geom="point", aes(color=seedmix), fun="mean", size=1.5)+
  stat_summary(data=subset(poll_divsum,   (management=="unmanaged"|(management!="unmanaged"&numericmonth<7))&numericmonth<7), geom="line", aes(color=seedmix), fun="mean", size=1)+
  stat_summary(data=subset(poll_divsum,   (management=="unmanaged"|(management!="unmanaged"&numericmonth<7))&numericmonth>5), geom="line", aes(color=seedmix), linetype=2, fun="mean", size=1)+
...
#model 1: within/across orchards seed mixes main effects
test<-subset(poll_divsum, management=="unmanaged"|(management!="unmanaged"&numericmonth<7))%>%
  mutate(trt=as.factor(paste(seedmix, orchard_age, sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block, numericmonth, sep="_")))%>%
  filter(management=="unmanaged"|(management!="unmanaged"&numericmonth<7))

mod<-lme(data=subset(test), richness~trt, random=~1|numericmonth/block)
...
#test 2: date by date
test<-subset(poll_divsum, management=="unmanaged"|(management!="unmanaged"&numericmonth<7))%>%
  mutate(trt=as.factor(paste( numericmonth, sep="_")))%>%ungroup()%>%
  mutate(id=as.factor(paste(orchard_age, block, sep="_")))%>%
  mutate(date=as.factor(numericmonth))

mod<-lme(data=subset(test, orchard_age==60&seedmix=="perennials"), richness~numericmonth, random=~1|block)

cld(glht(mod, mcp(date="Tukey")))
emmeans(mod, pairwise~numericmonth)
...
op2<-select(op1, -richness)%>%
  ...
  mutate(rem=paste(management, numericmonth, sep="_"))%>%
  filter(seedmix!="megamix")%>%
  filter(!rem%in%c("flailed_7", "flailed_8", "scraped_7", "scraped_8")) %>%
  mutate(rem=paste(management, seedmix, sep="_"))%>%
  filter(rem!="unmanaged_industry")
...
ggplot(subset(op2, !is.na(seedmix)), aes(x=as.numeric(numericmonth), y=abundance+1))+
 ...
 # scale_color_manual(values=c("#F8766D", "#00BA38", "#619CFF", "orange"))+
  ...
  stat_summary(data=subset(op2,   (management=="unmanaged"|(management!="unmanaged"&numericmonth<7))&numericmonth<7), geom="line", aes(color=seedmix), fun="mean", size=1)+
  stat_summary(data=subset(op2, (management=="unmanaged"|(management!="unmanaged"&numericmonth<7))&numericmonth>5), geom="line", aes(color=seedmix), linetype=2, fun="mean", size=1)+
  stat_summary(geom="point", aes(color=seedmix), fun="mean", size=1.5)+
...
mod<-lme(data=subset(test), abundance~seedmix, random=~1|numericmonth/block)
...
mod<-lme(data=subset(test, orchard_age==60&seedmix=="control"), abundance~numericmonth, random=~1|block)
